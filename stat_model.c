...
/*
Create strct Chrom. Consists of value and fit.
int value: domain
int fit: fitness parameter output from domain
*/
...
int main()  {	
    ...
    for(int i = 0; i <  6; i++) { //Initliaze each of the Chroms in current pop with random value from domain
        ...
        (current_pop[i])->value = (rand()%(MAX_VALUE - MIN_VALUE))-MAX_VALUE; //current domain is set to -100 to 100, can be changed
    }
    ...
    for(int i = 0; i <  num; i++) {  //run for num iterations
        ...
        for(int j = 0 ; j < 6; j++ ) {
            for(int k = 0; k < 3; k++) { //calculate each fitness value three times for each value
                temp = calculate_fitness((current_pop[j])->value);
                while(temp < LOWER_Y && temp > UPPER_Y) { //calculated fitness value is not in desired range
                    temp = calculate_fitness((current_pop[j])->value);    
                }
...
void mutate(int mutation_index) {
   ...
   /*  int bin = rand() % 2;
    ...
    printf("\n%s", "Mutation occured. Replacing Value. Value to mutate: ");
    printf("%d", (current_pop[mutation_index]->value)); 
